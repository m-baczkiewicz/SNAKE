...
class Snake(GameObject):
    def __init__(self):
        super().__init__(250, 250, "#088343", "snake")
        self.body_size = body_parts
        self.squares = []

        # Inicjalizacja początkowych współrzędnych węża
        for _ in range(body_parts):
            self.squares.append(self.draw(canvas, space_size))

class Food(GameObject):
    def __init__(self):
        super().__init__(0, 0, "#AE0964", "food")
        self.change_color()
        self.spawn_food()
...
def restart_game(event):
    global game_started, score, snake, food
    game_started = False  # Ustawienie stanu gry na nieuruchomiony po restarcie
    score = 0
    label.config(text="Score:{}".format(score))
    canvas.delete("all")
    snake = Snake()  # Utwórz nowy obiekt Snake
    food = Food()
    window.update()

def next_turn(snake, food):
    head_coordinates = snake.coordinates[0]
    if isinstance(head_coordinates, (list, tuple)):
        x, y = head_coordinates
    else:
        x, y = head_coordinates, 0
